...
|---|---|---|
| CPU/CUDA routing guard | `levels/execution.py` | `execution_light.py` -- the variable it tests does not exist there |
| `success` requires `passed > 0` | `desktop/cdp_driver.py` | `execution.py`, `execution_light.py` |
| Do not log the token | `cli/_git_auth.py` | `cli/docker/run.py` |
| The nodepack gate | `cli/_nodelink.py` | `cli/docker/run.py` (private fork) |

!!! danger "The 59-workflow bug is still live in `execution_light`"
    `execution.py` explains itself: *"Previously this case fell through
    ...
    `execution_light.py` never computes `other_list` -- the name is
...
`cdp_driver.py` is `_failed == 0 and _passed > 0` -- and the fix never
crossed back.

!!! warning "`comfy-test docker` prints your PAT into the CI log"
    `cli/_git_auth.py` defines `tokens_to_redact()`, reading exactly
    `GH_TOKEN`, `GITHUB_TOKEN` and `NODE_PAT`, with a docstring naming the
    hazard. Two call sites use it (`venv_server.py`,
    `windows_portable/platform.py`).

    `cli/docker/run.py` appends those same three variables *and their
    values* to `docker_cmd` prints the joined command. Same code at
    for Linux. GitHub masks secrets it issued to that job; a token
...
(, 55 lines against 44). The only textual difference is an added
...
that is not a nodepack. The original `copy_local_node` (`_nodelink.py`) now
has zero callers.
...
`with sync_playwright() as p:` block spanning -- 4.7% of the entire
...
(`_desktop_runner.py`). Exactly one place imports it as a module --
`_desktop_runner.py`, reaching for one helper -- and **that import runs
the whole desktop test**. Its `sys.exit(0)` in `cdp_driver.py` raises
...
checkout carry `if: github.event_name == 'push'`. **On a pull request it checks out the repo, skips everything, and reports
...
`orchestration/results.py` is `if os.environ.get("COMFY_TEST_CUDA"):` and
...
`common/config.py` has the same shape, and the docstring four lines above it
...
!!! danger "A `linux-cuda` dispatch runs as a Windows CPU job"
    `dispatch-test.yml` declares `lane`, and calls `platform` a
    *"DEPRECATED alias for `lane`"*. But `platform` is what every routing
    expression reads -- the job filter at and `runs-on` at --
    ...
    `test-matrix.yml` uses `${{ inputs.lane || inputs.platform }}` and is
...
`reproducibility.md` prescribes `comfy-test run --lane linux-cpu`, which
`argparse` rejects with exit 2. `commands.md` says, correctly, *"There is no
`--lane` flag."*
...
`tests/test_torch_triple.py` now **bans the string from reappearing**. It
says Python is *"drawn at random per run"*; `config.py` pins 3.13. The ADR
...
- **`orchestration/context.py`** kills the vacuous pass in as many words:
...
- **`levels/registration.py`** reports *"Imported cleanly but registered 0
  nodes"* -- a blind spot inherited straight from upstream, closed here.
- **`levels/coverage.py`** refuses a 0/0: *"this almost always means the
...
- **`common/resource_monitor.py`** omits metrics it could not measure
