...
|---|---|---|---|
| 1 | `free_memory` eviction ladder | <span class="v v-yes">yes</span> | When the card is short, ComfyUI walks its list of loaded models and tells them to leave, cheapest first. The stand-in comfy-env registers for each worker model is in that list, so the worker is told and unloads. See [the stand-in model](stand-in-model.md). |
