...
|---|---|---|---|
| 1 | `free_memory` eviction ladder | <span class="v v-yes">yes</span> | `free_memory` is asked for a number of bytes and makes them free: it walks `current_loaded_models`, sorted so the model already most offloaded goes first, then the least referenced, then the smallest, and unloads one at a time, re-reading free VRAM after each, until the request is covered or the list runs out. A model it never listed is never asked. The stand-in comfy-env registers for every model a worker holds is in that list, so a worker's model is asked in its turn and the worker does the real unload. See [the stand-in model](stand-in-model.md). |
| 2 | `load_models_gpu` admission | <span class="v v-yes">yes</span> | `load_models_gpu` is what every loader node calls. It adds up what the incoming models need (their size plus 10 percent, plus the larger of 0.8 GiB and the model's own activation estimate, plus the reserve), calls `free_memory` for that much, and only then loads. The one input comfy-env has to keep honest is free VRAM: on Linux it is device wide, so what workers hold is already counted; on Windows it is the calling process's own budget, so comfy-env publishes what workers hold into the reserve term. A worker about to load runs the same sum through the host, which frees room on its behalf. See [admission and the reserve](admission.md). |
