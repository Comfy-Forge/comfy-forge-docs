...
|---|---|---|---|
| 1 | `free_memory` eviction ladder | <span class="v v-yes">yes</span> | When the card is short, ComfyUI walks its list of loaded models and tells them to leave, cheapest first. The stand-in comfy-env registers for each worker model is in that list, so the worker is told and unloads. See [the stand-in](stand-in.md). |
| 2 | `load_models_gpu` admission | <span class="v v-yes">yes</span> | Before a load, ComfyUI adds up what it will need and checks free VRAM. On Linux free VRAM is device wide, so what workers hold is already counted; on Windows comfy-env publishes a reserve so that it is. See [admission and the reserve](admission.md). |
...
| 9 | `unload_all_models` and the Free button | <span class="v v-yes">yes</span> | The button reaches the stand-in and the worker releases everything. See [the stand-in](stand-in.md). |
...
| 12 | `LoadedModel` size questions | <span class="v v-partial">partial</span> | How big is it, how much is on the card. The stand-in answers from what the worker measured; some reads assume internals the stand-in only fakes. See [the stand-in](stand-in.md). |
...
| 17 | `model_unload`, partial versus full | <span class="v v-yes">yes</span> | The stand-in implements partial unload and returns the bytes actually moved, so ComfyUI's arithmetic stays right. See [the stand-in](stand-in.md). |
...
| 22 | `cleanup_models` prune | <span class="v v-yes">yes</span> | Dead entries are pruned; the stand-in carries the weakref the prune expects. See [the stand-in](stand-in.md). |
...
| 26 | `model_load` and the finalizer tripwire | <span class="v v-yes">yes</span> | The stand-in is inserted into the list directly and satisfies what the finalizer checks. See [the stand-in](stand-in.md). |
...
| 28 | Entry identity and the dead-entry sweep | <span class="v v-partial">partial</span> | The effect is right, the visibility is not: comfy-env must hold the stand-in's patcher strongly or the sweep drops it. See [the stand-in](stand-in.md). |
| 29 | Clone dedup and `is_clone` | <span class="v v-yes">yes</span> | Upstream probes the incoming host model with the stand-in as the argument, and the stand-in answers not a clone, which is the case that matters. Two of the three read paths assume internals. See [the stand-in](stand-in.md). |
| 30 | `is_dynamic` gate and the per-node ledger walk | <span class="v v-yes">yes</span> | The stand-in answers False and nothing deeper is read; the worker's own pager handles its models. See [the stand-in](stand-in.md). |
...
| 38 | `loaded_models()` leak into node code | <span class="v v-yes">yes</span> | Stand-ins register with `currently_used` False, so a node that calls `loaded_models()` does not pick them up. See [the stand-in](stand-in.md). |
...
| 40 | `unload_model_and_clones` | <span class="v v-yes">yes</span> | The stand-in's clone id is a private sentinel that never matches a real uuid. See [the stand-in](stand-in.md). |
