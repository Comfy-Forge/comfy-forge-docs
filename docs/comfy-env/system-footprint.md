...
| `<workspace>/install.log` | The transcript of the last workspace install that actually did work. A run where every env is already current returns before the log is opened, so this file can be older than your last install. | It is the first place to look when an env did not build; [`install()` / When it fails](install.md#when-it-fails) owns the full description. Kept beside the workspace rather than in the ComfyUI folder because the workspace is machine-wide and shared between installs (`workspace.py:717`). |
