...
| `contract.py` | 287 | The upstream coupling contract as data. Twenty-four entries with per-entry severity and tier (`floor` / `paged` / `shared`) and the version each appeared in, checked against the real tree at startup; a fatal gap refuses to start. Imports nothing; staged into workers. |
...
| `isolation/tensor_utils.py` | 64 | IPC preparation (clone a tensor whose storage came from another process so it can be re-exported), `release_tensor()` via `madvise(MADV_DONTNEED)`. |
