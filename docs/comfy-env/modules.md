...
| `workers/_persistent_worker.py` | 1714 | The worker program. Never imported by the parent -- read as text (`subprocess.py`) and run by the isolated interpreter with `_ipc_shared.py` copied alongside (the shared serialization core; the worker keeps only thin side-specific wrappers). Faulthandler + watchdog, object-reference cache, main loop. |
