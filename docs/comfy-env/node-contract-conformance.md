...
|---|---|---|---|---|
| 1 | `check_lazy_status` — **when you define one** | <span class="v v-yes">works</span> | A node can mark inputs as *lazy*: ComfyUI skips computing them up front and asks the node which ones it actually needs, then runs only those. A switch node uses this to execute one branch and leave the other untouched. Under isolation the question goes to the worker and the node's own answer comes back, so the unused branch stays uncomputed, same as native. One extra round trip per question, only if you wrote `check_lazy_status` | — |
