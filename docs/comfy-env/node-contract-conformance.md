...
| 2 | `__init__` and `self.x` (V1) | <span class="v v-yes">works</span> | **V1 nodes only** (a V3 node has no `self`; its `execute` is a classmethod). `self` is a backpack the host carries between calls; the worker puts it on, works, and hands it back — with the right types, and with in-place edits like `self.cache[k] = v` included. If the worker crashes or restarts, the node reinitializes: `__init__` runs again in the new process, and if the old backpack held something only the dead process could use (a thread, a lock, an open file) it starts from that fresh `__init__` state — the same as ComfyUI itself restarting | Nothing. A live handle on `self` is fine: `__init__` rebuilds it after a restart, and the rest of `self` is reset with it — the same thing plain ComfyUI does on its own restart |
