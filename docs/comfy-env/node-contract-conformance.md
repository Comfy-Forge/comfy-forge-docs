...
| 7 | Class attributes on **V1** nodes | <span class="v v-partial">partial</span> | `DESCRIPTION` and `OUTPUT_TOOLTIPS` are dropped, so tooltips and the help panel are blank; `SEARCH_ALIASES` is dropped, so the node is unfindable by synonym; `NOT_IDEMPOTENT` is dropped, so two copies in one graph share a cache entry. V3 nodes are unaffected — **unless** the V3 proxy build or the scan-side capture fails, in which case the node demotes to a V1 proxy and loses all ten. The proxy-build failure is announced: the host prints `[comfy-env] V3 proxy build failed for <node>, falling back to V1 proxy: <error>`. The scan-side capture failure (`GET_NODE_INFO_V1()` raising) is quiet unless `COMFY_ENV_DEBUG_META` is on, because the scan process's stderr is only replayed under that flag | Nothing available today. Consider a V3 node, where all of these are carried |
...
| 9 | `INPUT_TYPES` | <span class="v v-partial">partial</span> | **V1 combos** — an input spec shaped `(["a", "b"], {...})` — go live whenever that pack's worker is alive and idle, because the node's own `INPUT_TYPES` is re-run there, however it builds the list; they are frozen at scan values while the worker is cold or busy. **V3 `io.Combo.Input` options stay at the scan's values** whatever the worker is doing. The reason is a shape mismatch: a V3 combo reaches `INPUT_TYPES` as `("COMBO", {"options": [...]})`, and every site that refreshes or splices options — `_combo_input_names` and `_splice_combo_options` in `metadata.py`, `_combo_options` in the worker — recognises a combo only by a list in the entry's first position, so the V3 entry is passed over on both sides | V1: see [dynamic combos](live-dropdowns.md). V3: nothing today; a fix is planned |
...
| # | Mechanism | Status | What you will actually observe | What to do |
|---|---|---|---|---|
| D1 | `VALIDATE_INPUTS` / `validate_inputs` | <span class="v v-partial">deliberate</span> | A stand-in with the exemptions you declared is attached — every named argument the scan saw, and a `**kwargs` catch-all when you wrote one — so ComfyUI's built-in min/max and combo checks are waived for the same inputs as natively. On a **V1** proxy the stand-in also names **every combo input**, whether or not you wrote a validate at all: that is what lets a value from a live-listed dropdown reach the node, and it means upstream's not-in-list check is off for every combo on every isolated V1 node. V3 combos are not matched by that rule (their spec shape differs), so on a V3 proxy only your own exemptions apply. The **body** still never runs, so a rejection message never reaches the user: the node accepts the bad value and fails deeper in | Validate at the top of your node function and raise there. Name inputs explicitly rather than relying on `**kwargs` |
...
worker that already exists (row 13); `VALIDATE_INPUTS` stays deliberate (D1) and
