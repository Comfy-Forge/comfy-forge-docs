...
| 6 | `lock_class` (V3) | <span class="v v-no">not applied in the worker</span> | Nothing — and that is the problem. Writing `cls.foo = x` during execute succeeds and persists for the worker's life, where upstream raises. See the warning below | Do not write class state during execute. Test un-isolated before shipping |
