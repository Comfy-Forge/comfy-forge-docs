...
|---|---|---|---|---|
| 1 | `check_lazy_status` — **when you define one** | <span class="v v-yes">works</span> | You told ComfyUI "only compute the branch I pick". Your picker runs in the worker and its answer comes back, so only that branch is computed — same as native. It costs one extra round trip per decision, and it only happens if you actually wrote a picker | — |
