...
|---|---|---|---|---|
| 1 | `check_lazy_status` — **when you define one** | <span class="v v-yes">works</span> | A node can mark some of its inputs as *lazy*: ComfyUI then doesn't compute what feeds them up front. Instead it asks the node, "given what you have so far, which of these do you actually need?" — and only runs the upstream nodes that produce those. That is how a switch node works: it looks at its selector and asks for the one branch it's going to pass through, and the other branch never executes. Under isolation that question is forwarded to the worker and the node's own answer comes back, so the unused branch stays uncomputed — same as native. One extra round trip per question, and only if you wrote `check_lazy_status` yourself | — |
| 2 | `__init__` and `self.x` | <span class="v v-yes">works</span> | `self` is a backpack the host carries between calls; the worker puts it on, works, and hands it back — with the right types, and with in-place edits like `self.cache[k] = v` included. If the worker dies and a new one starts, `__init__` runs again there. If the old backpack held something only the dead process could use (a thread, a lock, an open file), the whole backpack is replaced by the fresh one, once, and the log says which item forced it; otherwise your old contents are put back | Nothing. A live handle on `self` is fine: `__init__` rebuilds it after a restart, and the rest of `self` is reset with it — the same thing plain ComfyUI does on its own restart |
| 3 | `PromptServer.instance.send_sync(...)` | <span class="v v-yes">works</span> | Your pack can `from server import PromptServer` and call `send_sync` from inside the worker, and the message reaches the browser through the host's real server. `sid=None` means everyone, anything else means the person who clicked Run. Sending from a background thread is dropped with one log line. Anything on the server that isn't `send_sync` / `send_progress_text` / `client_id` raises an error that tells you so (D4) | Nothing for events. For inbound HTTP, use [`ROUTES`](register-nodes.md) |
...
| 5 | `cls.SCHEMA` (V3) | <span class="v v-yes">works</span> | A V3 node that returns an expand graph used to crash because the worker never ran the setup ComfyUI does at registration. The worker now does that setup itself | — |
