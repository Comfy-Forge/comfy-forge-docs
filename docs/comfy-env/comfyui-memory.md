...
| M3 | **Node end call** | `reset_cast_buffers` (`model_management.py`) and `cleanup_prefetch_queues`, both from `execution.py` | When a node ends, the executor calls `reset_cast_buffers`, which frees, in order:<ul><li>the cast staging buffers, one per offload stream, torch's and aimdo's (<b>3 Carry</b>)</li><li>dirty mmap pages, bounced back so a written-through checkpoint page stops being charged to the process</li><li>cross-step state tensors parked on modules (a registry nothing writes to any more)</li><li>for every paged model: its pinned host memory for LoRA patches (<b>Model weights'</b> patch pins), and dead entries in its weight pin buckets</li><li>malloc graph records on modules</li><li>then `soft_empty_cache`: torch's idle allocator cache back to the driver (the reachable part of <b>6 Everything else</b>)</li></ul>and `cleanup_prefetch_queues`, which drops the prefetch queues and CUDA graph pools (<b>3 Carry</b> again). |
