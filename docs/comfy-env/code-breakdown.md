...
Where the lines actually go. **13,733 lines of Python across 39 files**
under `src/comfy_env/`.
...
| Subsystem | Lines | % |
|---|--:|--:|
| Transport / worker IPC | 4,879 | 36% |
| Environment build / install / wheels | 4,021 | 29% |
| Node registration / proxy / ComfyUI glue | 2,884 | 21% |
| Config / CLI / misc | 1,361 | 10% |
| Hardware detection | 588 | 4% |
| **Total** | **13,733** | 100% |
...
## Transport / worker IPC -- 4,879 lines (the biggest sink)

| File | Lines | What it is |
|---|--:|---|
| `isolation/workers/_persistent_worker.py` | 1,897 | The entire worker program, shipped to the far env as source text ([ADR-0006](adr/0006-worker-crosses-the-boundary-as-source-text.md)). Main loop, transport, faulthandler/watchdog, the by-reference object cache, the `print`/logger hijack. The single largest file, and it earns the shame: it is a whole program in one module. |
| `isolation/workers/subprocess.py` | 1,106 | Parent-side `SubprocessWorker`: spawn, authkey handshake, health, call/echo, consumed-ack, the canary + device-identity checks. |
| `isolation/workers/_ipc_parent.py` | 799 | Parent transport internals: `SocketTransport`, tensor strategies, `_from_shm`. |
| `isolation/workers/_ipc_shared.py` | 793 | The shared serialization core both sides import (the `_to_shm` walker, registry, `OpaquePayload`, and -- since 0.4.20 -- the CUDA-IPC forwarding cache, the one comfy_env-import-free leaf). |
| `isolation/tensor_utils.py` | 188 | `TensorKeeper`, madvise reclaim. |
...
## Environment build / install / wheels -- 4,021 lines

| File | Lines | What it is |
|---|--:|---|
| `install/workspace.py` | 972 | Workspace materialization: discover configs, resolve torch pin, pick wheel combo, hash for change detection, `pixi install` per env, stamp. |
...
| `environment/cache.py` | 616 | Env identity, ABI tags, workspace layout, the Windows LOCALAPPDATA guard. |
| `packages/cuda_wheels.py` | 480 | CUDA wheel index resolution ([ADR-0004](adr/0004-prebuilt-cuda-wheel-index.md)). |
| `environment/runtime.py` | 84 | The `RuntimeEnv` contract surfaced by `comfy-env info --json`: the 12 fields a consumer may depend on, so tools stop importing `_abi_tag` and hardcoding the env layout. |
| `environment/setup.py` | 74 | `setup_env()`: faulthandler, libomp dedupe. (Shrank from 214 in 0.4.22 when the parent-side shareable-pool hook was deleted, and again in 0.4.27 with the `base_directory` fill-in.) |
...
| `install/__init__.py` | 94 | `install()` entry. |
| `environment/libomp.py` | 137 | macOS libomp dedupe. |
| `packages/__init__.py` / `environment/__init__.py` / `install/verify.py` | 38 / 30 / 18 | Small. |
...
## Node registration / proxy / ComfyUI glue -- 2,884 lines

| File | Lines | What it is |
|---|--:|---|
| `isolation/metadata.py` | 1,269 | The scan subprocess + proxy synthesis ([ADR-0023](adr/0023-metadata-scan-and-proxy-synthesis.md)) -- the subsystem most exposed to ComfyUI schema churn (V1/V3 duality, DynamicCombo, hidden inputs). |
| `isolation/wrap.py` | 590 | `register_nodes()` orchestration -- was 1,112 until 0.4.20, when the worker pool and env builder were extracted (see below); now it reads like the one thing it is. |
| `isolation/pool.py` | 583 | The worker pool (0.4.20): lifecycle, restart+generations, VRAM/progress callbacks, route proxying, the `_STALE_PATCHERS` invariant ([ADR-0019](adr/0019-worker-lifecycle.md)). Extracted from `wrap.py` to break the `wrap`↔`metadata` cycle. |
...
| `detection/arch.py` | 18 | CPU architecture (`x86_64` / `aarch64`), which the tier-2 wheel fallback is keyed on. |
...
- **Delete-shaped work exists.** The transport duplication (~600-800
  doubled lines) and the monkey-patch surface (2,719 lines the upstream
...
- **`_persistent_worker.py` at 1,897 is the largest file, and it stays
