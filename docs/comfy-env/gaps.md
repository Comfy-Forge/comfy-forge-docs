...
| 39 | Latent / `ProgressBar` previews | The preview during sampling stayed blank. The worker fits the preview to its `max_size` the way `server.send_image` does, ships it on the progress callback and the host rebuilds it for `PROGRESS_BAR_HOOK`; tests drive ComfyUI's own `ProgressBar` from a worker with a 40x30 frame and a 2048x2048 one and check the tuple arrives fitted, and that a huge frame with no `max_size` arrives whole | **fixed** | [conformance](node-contract-conformance.md) | V |
...
| 57 | V3 node silently built as a V1 stand-in | When the host-side V3 builder raised, or the scan could not capture the V3 schema, comfy-env printed one line and built a V1-shaped stand-in: a node that loaded and ran and was a different node (no `DynamicCombo` expansion, V1 hidden inputs and wiring). Now a V3 node is built as V3 or the pack fails registration with a traceback, as a broken pack does natively. The 200-item cap on copied class attributes went with it: a scan of the top 500 packs (13,072 node classes) found nothing over 118 | **fixed** 2026-09-13 | [conformance](node-contract-conformance.md) | V |
